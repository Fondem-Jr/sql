...
SELECT name FROM actor
JOIN casting ON (actor.id = actorid AND (SELECT COUNT(ORD) FROM casting WHERE actor.id = actorid AND ORD = 1)>=15)
GROUP BY name;

-- List the films released in the year 1978 ordered by the number of actors in the cast, then by title. 

SELECT title,  COUNT(actorid) AS num_actor FROM movie
